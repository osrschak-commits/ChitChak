...
#
# Everything that matters is in Postgres - accounts, servers, channels, private
# conversations, and the avatars, which are rows rather than files. Redis holds
# only presence and rate-limit counters, which rebuild themselves, and the
# updates directory is build output that can be produced again. So this backs up
# one thing, and tries to do it properly.
#
...
log "$(find "$BACKUP_DIR" -maxdepth 1 -name 'chitchak-*.sql.gz' | wc -l) backups on disk"
